...
'''
This script produces Fig. S8 of Vallejo-Bernal & Braun et al., (2025)

Run this script from the command line with:

    python plot_figureS8.py
'''
...
year0 = 1979
yearf = 2023
ax_labels = [False, False, True]
perc = ['p85', 'p875', 'p90']
ylabels = ['85th percentile', '87.5th percentile', '90th percentile']
years = yearf - year0 + 1
central_lon = -150
color_map = cm.bilbao_r

# Folders and files
input_folder = root_folder + 'data/figureS8/'
figure_file = root_folder + 'manuscript/PIKART_FigureS8.png'
...
# Create the figure
figure = plt.figure(figsize=(180/25.4,240/25.4), dpi=600)
...
for i in range(3):
    ...
    ar_freq = np.load(input_folder + '%s-%s_%s_ar_freq.npy' %(year0, yearf, perc[i]))

    # AR landfall
    unique_lons = np.load(input_folder + '%s-%s_%s_unique_lons.npy' %(year0, yearf, perc[i]))
    unique_lats = np.load(input_folder + '%s-%s_%s_unique_lats.npy' %(year0, yearf, perc[i]))
    lf_map = np.load(input_folder + '%s-%s_%s_lf_map.npy' %(year0, yearf, perc[i]))/years
    ...
    ax = figure.add_subplot(gs[i], projection=proj)
    ax.set_global()
    ...
    lf_scatter = ax.scatter(pltlons[nonzero_mask], pltlats[nonzero_mask], transform=ccrs.PlateCarree(), zorder=5,
                            s=10*lf_map[nonzero_mask], alpha=.7, color='steelblue')

    # Maxfactor the map
    ax = maxfactor_fig10(ax, ll=True, bl=ax_labels[i])
    ax.set_aspect('equal')
    ...
    if i == 0:
        ...
        legend = ax.legend(handles, ['1', '5', '10', '15'], ncol=4, 
                           loc='upper left', bbox_to_anchor=(0.58, 1.1),
                           edgecolor='none', framealpha=1)
    
        ax.annotate('Land-falling ARs/year', xy=(30, 96), xycoords='data', 
                    ha='right', va='bottom')       
