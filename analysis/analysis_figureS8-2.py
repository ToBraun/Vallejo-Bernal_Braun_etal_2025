...
'''
This script computes the statistics necessary to plot Fig. S8 of 
Vallejo-Bernal & Braun et al., (2025).

Run this script from the command line with:

    python analysis_figureS8-2.py
'''
...
config = yaml.safe_load(open('config_PIKART_p85.yml'))
...
lagrangian_file = '1940-2023_PIKARTV1_p85_Vallejo-Bernal_Braun_etal_2025.csv'

output_folder = './output/figureS8/'
...
# Save results
np.save(output_folder + '%s-%s_p85_ar_freq.npy' %(years[0], years[-1]), ar_freq)
...
np.save(output_folder + '%s-%s_p85_unique_lons.npy' %(years[0], years[-1]), unique_lons)
np.save(output_folder + '%s-%s_p85_unique_lats.npy' %(years[0], years[-1]), unique_lats)
np.save(output_folder + '%s-%s_p85_lf_map.npy' %(years[0], years[-1]), a_lf_matrix)